...
def pos_eva():
    with open(os.path.join(DATA_DIR, 'pos_eva.txt'), encoding='utf-8', errors='ignore') as f:
        words = set([x.strip() for x in f])
        return words


def neg_eva():
    with open(os.path.join(DATA_DIR, 'neg_eva.txt'), encoding='utf-8', errors='ignore') as f:
        words = set([x.strip() for x in f])
        return words
...
def release():
    with open(output_path, "wb") as output:
        result = {}
        with open(os.path.join(DATA_DIR, 'pos.txt'), encoding='utf-8', errors='ignore') as f, \
                open(MAINTAINED_POS, encoding='utf-8', errors='ignore') as maintained_pos:
            pos_emotion = set([x.strip() for x in f]).union(
                [x.strip() for x in maintained_pos])

            result["pos_emotion"] = pos_emotion

        with open(os.path.join(DATA_DIR, 'neg.txt'),encoding='utf-8', errors='ignore') as f, \
                open(MAINTAINED_NEG,encoding='utf-8', errors='ignore') as maintained_neg:
            neg_emotion = set([x.strip() for x in f]).union(
                [x.strip() for x in maintained_neg])

            result["neg_emotion"] = neg_emotion

        with open(os.path.join(DATA_DIR, 'pos_tc.txt'), encoding='utf-8', errors='ignore') as f:
            pos_emotion = set([x.strip() for x in f])

            result["pos_emotion_tc"] = pos_emotion

        with open(os.path.join(DATA_DIR, 'neg_tc.txt'), encoding='utf-8', errors='ignore') as f:
            neg_emotion = set([x.strip() for x in f])

            result["neg_emotion_tc"] = neg_emotion

        with open(os.path.join(DATA_DIR, 'pos_sentence.txt'), encoding='utf-8', errors='ignore') as f:
            pos_sentence = set([x.strip() for x in f])
            result["pos_sentence"] = pos_sentence

        with open(os.path.join(DATA_DIR, 'neg_sentence.txt'), encoding='utf-8', errors='ignore') as f:
        ...
        with open(os.path.join(DATA_DIR, "degrees_copy.json"), encoding='utf-8', errors='ignore') as f:
            degrees = json.load(f)
            result["degrees"] = degrees

        negations = set(
            json.load(open(os.path.join(DATA_DIR, 'negations.json'), encoding='utf-8', errors='ignore')))
